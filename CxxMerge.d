  ...
  struct Result
  {
	...
	void popFront()
	{
	  ...
	  if (!tokenRanges_[pos_].empty)
	  {
		...
		{
		  auto states = iota(rangeState_.length).zip(rangeState_).cycle(pos_+1).take(rangeState_.length).array;
		  ...
		  else
		  {
			...
			if (!p.empty)
			{
			  ...
			  //std.stdio.writeln(">>> ", name, " ", pos_, " ", rangeState_, " ", states, " <<<");
			}
			else
			{
			  //nextPosition();
	...
	alias Tuple!(string, "name", string, "action") RangeState;
